...
if config.user_study:
	from utils.sequence_visualizer import SequenceVisualizer
from utils.sequence_visualizer import SequenceVisualizer
...
if config.user_study:
	vis_root = config.vis_root + '/ourgan' + config.labels_type
	# vis_root = config.vis_root + '/ourgan' + config.labels_type + '_gt'
...
if config.dataset_name == 'H36M_less_joints' or config.dataset_name == 'H36M_less_joints_fixed':
	ACTIONS = H36M.ACTIONS
	ITEMS = np.array([1, 3, 7, 9, 13, 15, 17, 24, 49, 74, 99])
...
for action in ACTIONS:
	...
	n_output_lables = config.n_in + config.n_out*(auto_regress_vis+1) + config.forecast
	...
	if config.labels_type_ablation == 'onlygan' and config.dataset_name == 'H36M' and config.dataset_type == 'Euler':
	...
	if config.labels_type == 'clustered' and config.dataset_name == 'H36M' and config.dataset_type == 'Euler' and config.label_dim == 12:
	...
	if config.plot_videos:
		...
		if config.user_study:
			vis = SequenceVisualizer(vis_root, action, to_file=True)
		...
		for i in indices:
			...
			if config.user_study:
				vis.plot(seq1=x, seq2=y_hat, name= str(i), video_fps=25, create_video=True)
				vis.plot(seq1=x, seq2=y, name= str(i), video_fps=25, create_video=True) # For GT visualization
			else:
				vis.plot(seq1=x, seq2=y_hat, name=action + str(i), video_fps=25, create_video=True)

				# vis.plot_in_a_image(seq1=Seq_match[1::5], create_video=True, video_fps=25, name='vis_', noclear=True,
				#                     noaxis=True)
	...
	if config.dataset_type == 'Euler':
		...
		# Y_Euler = Y_Euler[:,:config.frames_to_visualize]

...
if config.dataset_name == 'H36M':
	...
	ACTIONS = ['discussion', 'eating', 'walking', 'posing', 'smoking']
	ACTIONS = ['posing']
...
for action in ACTIONS:
	...
	for noise_number in range(config.number_of_noises):
		...
		noise = torch.randn(1, config.num_seeds, config.noise_dim, device=config.device) * 8
	...
	# for number_of_sequence in range(8):
	for number_of_sequence in [4]:
		vis = SequenceVisualizer(vis_root, action + str(number_of_sequence), to_file=True)

		for noise_n in range(30):
			seqin = X_all_noise[noise_n, number_of_sequence]
			seq = Y_hat_all_noise[noise_n, number_of_sequence]
			vis.plot_in_a_image_file(seq1=seqin[1::5], seq2=seq[1::5], create_video=True, video_fps=25, name=noise_n, noclear=True, noaxis=True)
